...
class CRUDUser(CRUDBase[User, UserCreate, UserSchema]):    
    ...
    async def save_avatar(self, sfp: Path, *, user: UserSchema, file_id: str, filename: str):
        ...
        ffp = Path(settings.IMAGES_LOCAL_DIR) / self._table.build_avatar_filepath(user.id, new_filename)
