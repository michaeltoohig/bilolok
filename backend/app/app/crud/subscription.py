...
from app.schemas.subscription import SubscriptionSchemaIn, SubscriptionSchema


class CRUDSubscription(CRUDBase[Subscription, SubscriptionSchemaIn, SubscriptionSchema]):    
    @property
    ...
    async def create(self, in_schema: SubscriptionSchemaIn, *, user_id: UUID) -> SubscriptionSchema:
        item_id = uuid4()
    ...
    async def find_by_device_id(self, device_id: str, *, user_id: UUID) -> SubscriptionSchema:
        query = (
            select(self._table)
            .filter(
                and_(
